...
for trace in namelist:
    ...
    with open('4waySA_512kb_2byte_{}_trace.txt'.format(trace),'w') as w:
    ...
    cache_table=[]
print(result_table)
with open('BlockComparison.txt','w') as w:
    w.write(str("4waySA_4byte"))
    w.write("\n")
    w.write(str(result_table))
    w.write("\n")
end = time.time()
